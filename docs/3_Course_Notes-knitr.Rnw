...
%Creating an \boldblue{R} Project is straightforward: select \texttt{File > New Project} and follow the instructions.
...
{.}\\\\
The object \texttt{Peru\_Soil\_Data} is a type of object known as a data frame. You can explore the data visually by clicking on its entry in the Environment tab. Alternatively, there are functions in base \boldblue{R} that are good for telling you what your data look like. For data frames, these include \code{head()} and \code{str()}. Try these out.
...
Exploring and manipulating data is fundamental to data analysis. In this section, we will briefly cover how to sort and filter the soil dataset. There are several approaches to doing this in the base code of \boldblue{R}, but here we will use on the functions \code{select()}, \code{filter()} and \code{arrange()} from the package \textbf{\texttt{dplyr}}. \\
...
#pdf("figs/base_graphs.pdf", width = 8, height = 3)
#par(mfrow = c(1, 3))
